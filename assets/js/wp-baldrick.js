        ...
        if(typeof window[params.url] === 'function'){
          ...
          try{
            if( $(params.url).length ){
              var dt = $(params.url).is('input,select,radio,checkbox,file,textarea') ? $(params.url).val() : ( $(params.url).is('form') ? $(params.url).formJSON() : $(params.url).html() );
            }
          }catch (e){}
...
(function($){
  ...
  $.fn.baldrick.registerhelper('baldrick_modal', {
    ...
    event : function(el, obj){
      var trigger = $(el), modal_id = 'wm', modal_element = 'form';
      if(trigger.data('modal') && wm_hasModal === false){
        if(trigger.data('modalElement')){
          modal_element = trigger.data('modalElement');
        }
