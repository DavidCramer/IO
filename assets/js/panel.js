...
function cf_io_randomUUID() {
		var s = [], itoh = '0123456789ABCDEF';
		for (var i = 0; i <6; i++) s[i] = Math.floor(Math.random()*0x10);
		return s.join('');
}
var cf_io_field_callbacks = [];
jQuery('document').ready(function($){
	// add row
	$('body').on('click', '.cf-io-add-group-row', function(){
		var clicked = $( this ),
			rowid = cf_io_randomUUID(),
			...
			for(var callback in cf_io_field_callbacks){
				if( typeof window[cf_io_field_callbacks[callback]] === 'function'){
					window[cf_io_field_callbacks[callback]]();
				}
			}

	});
	$('body').on('click', '.cf-io-removeRow', function(){
		$(this).next().remove();
		$(this).remove();
		////console.log(this);
	});
	// tabs
	$('body').on('click', '.cf-io-metabox-config-nav li a, .cf-io-shortcode-config-nav li a, .cf-io-settings-config-nav li a, .cf-io-widget-config-nav li a', function(){
